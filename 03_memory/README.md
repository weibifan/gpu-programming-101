...
先加载 MSVC 环境（否则 `nvcc` 找不到 `cl.exe`，见 `第2章 环境搭建.md` §3.3）：
...
* 共享内存的 tile 行长建议 `TILE+1`（padding），避免 32 的倍数造成 bank conflict（详见 `第4章 CUDA内存与性能优化.md` §3.4 / §6）。

对应文档：`第4章 CUDA内存与性能优化.md` §3、§5、§6。
