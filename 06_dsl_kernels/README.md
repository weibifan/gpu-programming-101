...
目标：把 `第4章 CUDA内存与性能优化.md` 的 tiling、`第6章 LLM加速.md` 的 FlashAttention，用 **Triton**（Python 语法）各写一遍，理解 kernel 编写思路。语法与逐行讲解见 **`第7章 用Python写高性能内核.md`**。
...
* `sgemm.py` 就是 CUDA 性能篇 `naive → shared → tiling` 的最终形态：一个 Python kernel 就拿到了接近 cuBLAS 的性能（通常 7~9 成）。
* `flash_attention.py` 是 LLM 篇 §3 的落地：S/P 矩阵从不出共享内存，全局内存读写 O(T²)→O(T)。
* 手写内核的意义是**看懂报告/理解原理**；生产环境直接用 cuBLAS / FlashAttention（`08_cuda_libs/`、`F.scaled_dot_product_attention`）。
