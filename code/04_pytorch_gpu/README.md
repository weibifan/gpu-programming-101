...
目标：把 `docs/04_pytorch_gpu.md` 里的 PyTorch GPU 手段逐个跑一遍。
...
| `ex22_minigpt.py` | 极简 decoder-only Transformer（玩具 GPT）：embedding→注意力→block→组装→自回归生成 | 附件3（Transformer 架构入门） |
...
* AMP 与量化都是"省字节 = 省带宽"（`docs/03_cuda_advanced.md` §9 的 memory-bound 直觉）；`torch.compile` 是"算子融合"的自动化版。
* Transformer 全家桶（token→embedding→注意力→Block→组装→自回归生成）见 `ex22_minigpt.py`，配套文档 `docs/附件3_Transformer 架构入门.md`；05 篇的 KV cache / FlashAttention 优化的正是里面的组件。
