# 附件1：Python 语法速查（写给 C / Java 程序员）

> 定位：你已熟悉 C 或 Java，本附件**不讲"编程是什么"**，只讲"Python 哪里不一样"。学完应能看懂 04 篇里的 `class`、`@`、`with`、`yield`、`lambda`、列表推导式这些写法，以及 §9 的 NumPy / SciPy 数组（张量的前置）。
>
...
PyTorch 里常见：`with torch.inference_mode():`——`__enter__` 关掉 Autograd 记账，`__exit__` 恢复现场（04 篇 §3.4）。
...
> **为什么 PyTorch 的张量能直接 `a + b`、`a * b`？** 因为 `torch.Tensor` 重载了全套运算符（`__add__`、`__mul__`…），所以 `x + y` 这个"像普通数字"的写法，实际会走张量的运算逻辑（04 篇 §2.2）。这就是运算符重载在真实库里的价值。
...
* 科学计算有固定别名约定：`import numpy as np`——看到 `np.xxx` 就是 NumPy（本仓库案例、04 篇大量出现）
...
> 深度学习的"数据"是**多维数值数组**：图片是 `[高, 宽, 通道]` 的数字、权重是一张数字表。Python 原生 `list` 能用，但没有"形状"的概念、运算也慢。**NumPy** 提供 `ndarray`（n 维数组），PyTorch 的张量（04 篇 §2）就是它的"GPU 亲戚"——`torch.Tensor` 和 `ndarray` 长得几乎一样。先把 NumPy 的基本直觉建立起来，04 篇上手会非常顺。
...
* `(3,)` 和 `(3, 1)` 都是"3 个数"，但**形状不同**：前者是一维数组，后者是 3 行 1 列的"矩阵"。张量编程里形状写错最常见就栽在这——04 篇的图片张量 `[3, 224, 224]` 是"通道在前"的写法
...
> 这个区别一路带到 PyTorch：`x * y` 是逐元素、`x @ y` 是矩阵乘（04 篇 §2.2）。
...
* 图片处理常要"换轴"：`img.swapaxes(0, 2)` 交换第 0 和第 2 维（把 RGB 从最后一维挪到最前），再 `reshape(1, 3, 256, 256)` 凑出"批 × 通道 × 高 × 宽"的四维张量——04 篇数据加载就是这套
...
> `np.asarray(x, dtype=...)` 把已有的 list/图片"变成"数组，`dtype` 指定元素类型——`float32` 是深度学习最常用的精度（对应 00 篇 §2.2 的 FP32）。
...
> PyTorch 里 `nn.Conv2d` 做的也是"图像 × 核 = 特征图"，只是核是**训练学出来的**而不是手写的。04 篇你会看到：卷积这类高频算子最终交给 cuDNN 优化（autotuner 选最快算法），数据也常摆成对卷积更友好的 `NCHW` 布局（对应 §9.3 的 CHW 四维张量）——这条直觉读 04 篇会顺很多。
